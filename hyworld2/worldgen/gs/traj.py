...
def generate_bspline_path(
    ...
    """
    使用 B-Spline 对相机轨迹进行平滑插值。

    对平移分量直接做 B-Spline 插值，对旋转分量使用 Slerp 插值后
    再用 B-Spline 平滑欧拉角（避免万向锁问题通过旋转矩阵重建）。

    Args:
        cameras: (N, 3, 4) 相机外参矩阵 [R | t]
        n_points: 输出轨迹点数
        degree: B-Spline 阶数，默认3（三次B样条）
        smoothness: 平滑因子，0表示精确插值，越大越平滑

    Returns:
        smooth_cameras: (n_points, 3, 4) 平滑后的相机外参
    """
    from scipy.interpolate import make_interp_spline
    from scipy.spatial.transform import Rotation, Slerp

    N = cameras.shape[0]
    assert N >= degree + 1, f"B-Spline degree {degree} 需要至少 {degree + 1} 个控制点，但只有 {N} 个"

    # 参数化：按弧长近似归一化到 [0, 1]
    translations = cameras[:, :3, 3]  # (N, 3)
    diffs = np.linalg.norm(np.diff(translations, axis=0), axis=1)  # (N-1,)
    cum_dist = np.concatenate([[0], np.cumsum(diffs)])
    t_knots = cum_dist / cum_dist[-1]  # 归一化到 [0, 1]

    t_query = np.linspace(0, 1, n_points)
    ...
    if smoothness > 0:
        ...
        tck, _ = splprep(
            ...
            u=t_knots, k=degree, s=smoothness
        )
        smooth_trans = np.array(splev(t_query, tck)).T  # (n_points, 3)
    else:
        spline_x = make_interp_spline(t_knots, translations[:, 0], k=degree)
        spline_y = make_interp_spline(t_knots, translations[:, 1], k=degree)
        spline_z = make_interp_spline(t_knots, translations[:, 2], k=degree)
        smooth_trans = np.stack(
            [spline_x(t_query), spline_y(t_query), spline_z(t_query)], axis=-1
        )  # (n_points, 3)

    # ============ 2. Slerp 插值旋转 ============
    rotations = Rotation.from_matrix(cameras[:, :3, :3])  # (N,)
    slerp = Slerp(t_knots, rotations)
    smooth_rots = slerp(t_query)  # (n_points,)

    # 可选：对旋转也做 B-Spline 平滑（通过旋转向量空间）
    if smoothness > 0:
        rotvecs = smooth_rots.as_rotvec()  # (n_points, 3)
        t_query2 = np.linspace(0, 1, n_points)
        spline_rv0 = make_interp_spline(t_query2, rotvecs[:, 0], k=degree)
        spline_rv1 = make_interp_spline(t_query2, rotvecs[:, 1], k=degree)
        spline_rv2 = make_interp_spline(t_query2, rotvecs[:, 2], k=degree)
        smooth_rotvecs = np.stack(
            [spline_rv0(t_query2), spline_rv1(t_query2), spline_rv2(t_query2)], axis=-1
        )
        smooth_rots = Rotation.from_rotvec(smooth_rotvecs)

    # ============ 3. 组装输出 ============
    smooth_cameras = np.zeros((n_points, 3, 4), dtype=np.float32)
    smooth_cameras[:, :3, :3] = smooth_rots.as_matrix()
    smooth_cameras[:, :3, 3] = smooth_trans
